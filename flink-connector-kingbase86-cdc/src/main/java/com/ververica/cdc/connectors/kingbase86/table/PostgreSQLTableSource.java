...
package com.ververica.cdc.connectors.kingbase86.table;

import com.ververica.cdc.connectors.kingbase86.PostgreSQLSource;
...
public class PostgreSQLTableSource implements ScanTableSource, SupportsReadingMetadata {
    ...
    public ScanRuntimeProvider getScanRuntimeProvider(ScanContext scanContext) {
        ...
        if(dbzProperties.containsKey("decimal.format")){
            customConfig.put("decimal.format",dbzProperties.getProperty("decimal.format").toUpperCase());
        }
        if (mode.equals("cdas")) {
            deserializer = new WrapRowDataDebeziumDeserializeSchema(physicalDataType, typeInfo, new JsonDebeziumDeserializationSchema(true,customConfig));
//            dbzProperties.setProperty("signal.data.collection","PUBLIC.debezium_signal1");
...
//                        .schemaList(schemaName)
                        .tableList(mode.equals("cdas") && dbzProperties.containsKey("signal.data.collection")?tableName + "," + dbzProperties.getProperty("signal.data.collection"):tableName)
                        .username(username)
                        .password(password)
